...
- **Уведомления унифицированы**: все сообщения (успех, ошибка, предупреждение, подтверждение) используют один дизайн (centre, blur, icon, anima).
- **Оператор ШИГ/КУГ (gmcActor)**: обязательное поле, записывается в аудит-лог, блокирует действия формы GMC до заполнения.
- Для поля оператора ШИГ/КУГ добавлен тестовый список автоподсказок (datalist); это только помощь при вводе и не отменяет правило фиксации оператора на уровне конкретной заявки.
- Возвратный маршрут через ШИГ/КУГ выведен из рабочего контура: неодобренные Комитетом заявки переводятся в `postponed` к Фасилитатору (3 месяца), ШИГ/КУГ видит их только в режиме просмотра.
...
| Отложена | `postponed` | Фасилитатор | 3-е неодобрение в ШИГ/КУГ ИЛИ отклонение Комитетом: заявка блокируется на 3 месяца, затем получает признак готовности к разблокировке (unlock-ready) |
...
stateDiagram-v2
    ...
    piu_review --> fac_revision: ГРП вернул с замечаниями
    ...
    com_review --> postponed: Комитет отклонил (блокировка 3 мес.)
...
| `piu_review` | `fac_revision` | ГРП | Есть замечания |
...
| `com_review` | `postponed` | Комитет | Решение "Отклонить" + обязательный комментарий (блокировка 3 месяца) |
...
| Возврат с замечаниями к Фасилитатору | Требует новой Word-версии |
...
flowchart LR
    ...
    D[ГРП/Комитет вернул с замечаниями] --> E[Фасилитатор загружает Word V2+]
    E --> F[Повторная отправка по процессу]
