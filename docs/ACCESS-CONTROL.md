...
- Toast-уведомления переработаны визуально: увеличенный формат, белый фон, синяя типографика и карточный bubble-стиль в правом нижнем углу.
- В уведомлениях о смене статуса добавлен явный маршрут заявки: кому передано действие и какой следующий статус установлен (`Маршрут: ... -> ...`).
- Для сценариев без смены статуса добавлена явная пометка `Маршрут без изменений` (например, блокировка `postponed` до даты), а для одиночных решений Комитета также показывается целевое направление заявки.
- Тексты статусных уведомлений приведены к единому шаблону: `Что произошло` -> `Маршрут` -> `Следующий статус`.
- В пользовательских уведомлениях маршрутов используются понятные названия этапов без технических англоязычных кодов статусов.
- Все уведомления `AppNotify` показываются в едином центре экрана, чтобы стиль не отличался между сценариями.
- Для сообщений маршрутизации заявок добавлена усиленная иерархия: `Что произошло`, `Маршрут`, `Следующий статус`.
...
- Добавлен отдельный режим `Финансовый отдел`: отображает только полностью завершенные заявки и предоставляет выгрузку выписки для бухгалтерии.
