...
- **Система уведомлений полностью переработана**: все сообщения (уведомления, ошибки, подтверждения) теперь отображаются в **едином дизайне** по центру экрана с размытым фоном, круглой иконкой, текстом и кнопкой(ми) действия.
- **Типы сообщений**: Success (✓ зелёная иконка, автоклозинг 4с), Info (i синяя, автоклозинг 5с), Warning (! янтарная, требует закрытия), Error (! красная, требует закрытия), Confirm (? янтарная, две кнопки).
- **Поле ФИО оператора ШИГ/КУГ**: обязательное, не пустой ввод блокирует кнопки решений (`Рекомендовать`, `На доработку`, `Отклонить`), валидация в реальном времени через `input` event-listener, автоматически заполняется при загрузке формы, read-only при закрытых статусах, используется в аудит-логе как `gmcActor`.
- Тексты статусных уведомлений: единый формат `Что произошло → Маршрут → Следующий статус`, без технических кодов статусов.
- В UI введён отдельный признак полной завершенности: заявка считается полностью завершённой, если она находится в `approved` и по ней загружен подписанный договор.
- В фильтрах `По статусам` и `Фасилитатор` пункт `Полностью завершенные` показывает только такие заявки; `approved` без договора остаются в обычной группе одобренных.
...
| Статус заявки | Фасилитатор | ШИГ / КУГ | Комитет |
|---|:---:|:---:|:---:|
| `draft` | ✅ **Редактирование** | 👁 Просмотр | 👁 Просмотр |
| `incomplete_data` | ✅ **Редактирование** (отправка заблокирована) | 👁 Просмотр | 👁 Просмотр |
| `fac_revision` | ✅ **Доработка** | 👁 Просмотр | 👁 Просмотр |
| `gmc_review` | 👁 Просмотр | ✅ **Скоринг** | 👁 Просмотр |
| `gmc_preparation` | 👁 Просмотр | ✅ **Подготовка реестра** | 👁 Просмотр |
| `gmc_ready_for_registry` | 👁 Просмотр | ✅ **Формирование реестра** | 👁 Просмотр |
| `com_review` | 👁 Просмотр | 👁 Просмотр | ✅ **Утверждение** |
| `approved` | 👁 + 📝 Черновик договора + 📎 Договор + 📊 Мониторинг | 👁 Просмотр | 👁 Просмотр |
| `rejected` | 👁 История | 👁 История | 👁 История |
| `postponed` | ✅ **Разблокировка/история** (если срок блокировки истек) | 👁 История | 👁 История |

Примечание:
Полностью завершенная заявка не переводится в новый технический `status`; это визуальный и фильтрационный признак для `approved`-заявки с уже загруженным подписанным договором.

Режим `Финансовый отдел` использует ту же базу заявок, что и раздел одобренных Комитетом, но исключает записи без прикрепленного подписанного договора.
...
После загрузки подписанного договора заявка попадает в точный фильтр полностью завершенных и выделяется отдельным статусным бейджем в карточках и таблицах.

Для бухгалтерии доступна выгрузка `CSV` по полностью завершенным заявкам с полями: ID, ФИО, сектор, сумма, протокол, метаданные договора.

Примечание по документам Фасилитатора:
- Word-документ не блокирует отправку и считается опциональным.
- Для базовой отправки используются обязательные PDF + 4 фото.
- Если Word загружен, система сохраняет факт загрузки и ведет версионность (`Word V*`).
...
| Рекомендовать к подготовке реестра | `saveGmcDecision()` (ok) | Балл ≥ 45 |
...
| Просмотр отложенных заявок | `renderAllCards()` (режим GMC) | Статус `postponed`, только чтение |

Дополнение по валидации опросника ШИГ/КУГ:
- В `saveGmcDecision()` включена строгая обязательность всех пунктов анкеты (`el1-el3`, `q1-q15`).
- До полного заполнения анкеты (`100%`) кнопки выбора решения в форме ШИГ/КУГ заблокированы.
- При пропуске хотя бы одного пункта сохранение блокируется, проблемные строки подсвечиваются красным, экран прокручивается к первой ошибке.
...
Форма ШИГ автоматически переключается в read-only, когда заявка находится вне зоны активной работы роли.
...
| `postponed` | 🔒 Read-only | Все radio, select, comment, кнопки решений |
...
| Рекомендован к подготовке реестра | ✅ Toast (success/info) | `'Тасдиқ шуд, барои омодасозии реестр гузашт. / Одобрено, передано на подготовку реестра.'` |
...
graph TB
    ...
    subgraph "ШИГ / КУГ"
        UC7[Скоринг заявки]
        UC8[Рекомендация к подготовке реестра]
        UC9[Возврат на доработку]
        UC10[Отклонение заявки]
        UC11[Анализ возврата Комитета]
