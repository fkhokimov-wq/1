...
| Отправить в ШИГ / КУГ | `submitToGmc()` | Заполнены сектор + сумма, `isComplete === true`. Если неполные → alert + return (кнопка `#btn-submit-facilitator` disabled) |
...
| Клик на карточку | Для `approved/rejected`: открывает read-only. Для остальных чужих статусов: alert «Ин марҳила кори [Роль] нест» |
...
| Рекомендован в ГТЛ / ГРП | ✅ Alert (info) | `'Тасдиқ шуд ва ба ГТЛ равон шуд. / Утверждено и направлено в ГРП.'` |
