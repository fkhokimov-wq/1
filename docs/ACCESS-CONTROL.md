...
- Маршрутные уведомления (пересылка заявки между ролями) показываются по центру экрана для более заметного подтверждения действия.
- Для центральных маршрутных уведомлений добавлена усиленная иерархия: крупный заголовок, отдельная акцентная строка `Маршрут`, отдельная строка `Следующий статус`, увеличенная ширина карточки.
