...
- **Унифицированная система уведомлений**: все сообщения (toast + confirm) отображаются по центру экрана с размытым background (`backdrop-filter: blur(4px)`), круглой цветной иконкой (✓ зелёная, ! красная/янтарная, i синяя, ? янтарная), анимацией `scale(0.9 → 1)`.
- **Поле ФИО оператора ШИГ/КУГ**: обязательное поле в форме GMC, блокирует все кнопки решений и действий до заполнения, значение сохраняется в аудит-лог вместо обобщённого `'ШИГ / КУГ'`.
- **Главное меню фильтров**: выланцировано на `flex: 1 1 0` (все 7 кнопок — равная ширина), удалены KPI-счётчики (135/30/1) справа, все кнопки одинаковой высоты (74px), бейджи маленькие (9px, 0 padding вверх-вниз).
- **Toast-уведомления**: успех и информация закрываются автоматически (4–5 сек), предупреждения и ошибки требуют ручного закрытия (кнопка или клик на фон).
- Confirm-диалоги: иконка `?` янтарная, две кнопки, Escape закрывает.
