...
- Добавлен отдельный onboarding-блок `Маршруты статусов за 30 секунд` в верхней части FAQ: компактная схема без кодовых названий статусов и внутренней терминологии.
- FAQ перестроен в role-centric формате по образцу help-center навигации: вместо хеш-тегов используется блок выбора роли с конкретными действиями пользователя по заявке.
- Внутренние ссылки FAQ (карточки ролей, быстрые кнопки, переходы помощи) синхронизированы с ролевыми фильтрами, чтобы переход всегда открывал релевантный контент. Добавлен отдельный расширенный блок инструкций для каждой роли.
