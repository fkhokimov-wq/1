...
│ 📝 Имя Фамилия                     │
│    INН: 1234567890                  │
│    Район: названиерайона           │
│ ┌───────────────┐                   │
│ │  Статус-бейдж │  Черновик         │
│ └───────────────┘                   │
│ [🔄 Кнопка действия / Просмотр]    │
│                                     │
│ Дата: 15.03.2025 14:30             │
└─────────────────────────────────────┘
```
...
| ШИГ | `gmc` | 15 критериев скоринга (1–4 балла каждый), 3 критерия соответствия (yes/no), комментарий, решение | ✏️ Заполняет ШИГ |
| ГРП | `piu` | Чек-лист верификации (1 шаг), комментарий | ✏️ Заполняет PIU |
...
Если хотя бы одно не заполнено → alert: `'Лутфан ҳамаи майдонҳои ҳатмиро пур кунед!'`
...
2. **Alert**: `'Огоҳинома ба Администратор фиристода шуд!'`
...
| postponed | Серый | `bg-gray-200 text-gray-600` |
...
Весь текст дублируется с использованием CSS:

```html
Маъмурият <span class="ru">/ Администрация</span>
```

Класс `.ru` имеет стиль `opacity: 0.5` и меньший шрифт — русский текст виден, но визуально вторичен.

Для блочных элементов: `<div class="ru-block">Русский текст</div>`.
...
Скачивается тектовый файл с информацией о заявке, секторе и бизнес-плане.
