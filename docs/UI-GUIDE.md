...
- **Доработка-бейдж**: компактный инлайн `<span>` с иконкой `refresh-cw` и текстом `1/3` (при `revisionCount > 0` и статусах `fac_revision`, `postponed`). Полный текст в `title`: «Миқдори такмил / Доработка: 1/3»
...
- **Оповещения (alert)**: все alert-сообщения
...
Скачивается текстовый файл с двуязычной информацией о заявке:

```
НАҚШАИ ТИҶОРАТӢ / БИЗНЕС-ПЛАН
ID: 10040
Аризадиҳанда / Заявитель: Имя
Бахш / Сектор: Сектор
Маблағ / Сумма: 25 000 сомонӣ
Сана / Дата: 2024-01-15
```
